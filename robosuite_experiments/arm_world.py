...
class ArmWorld:
    ...
    def __init__(
        ...
        self.zone_a_pos = np.array([-0.2, -0.3, 0.82])  # Left side of table (red block spawns here)
        self.zone_b_pos = np.array([0.2, 0.3, 0.82])    # Right side of table (target zone)
        self.zone_c_pos = np.array([-0.15, 0.25, 0.82])  # Zone C (obstacle zone)
