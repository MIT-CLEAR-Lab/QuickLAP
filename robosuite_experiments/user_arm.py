...
class UserArm(BaseRationalArm):
    ...
    def features(self, obs):
        ...
        """
        return feature_utils.compute_features(
            obs, include_red_dist=False, include_zones=True
        )
    ...
    def is_intervention(self):
        ...
        return self.physical_intervention_active
    ...
    def signal_physical_intervention(self, obs, robot_action, human_action):
        ...
        if not self.physical_intervention_active:
            ...
            print(
                f"[Timestep {self.timestep}] Physical intervention started (provide utterance when done)"
            )
        ...
        robot_obs["red_block_pos"] = self.physical_robot_sim_state[
            "red_block_pos"
        ].copy()

        # Record trajectories
        # Robot trajectory: counterfactual state (simulated without human input)
        self.robot_trajectory.append({"obs": robot_obs, "control": robot_action.copy()})
        # Human trajectory: actual state (with human input applied)
        self.human_trajectory.append(
            {"obs": copy.deepcopy(obs), "control": human_action.copy()}
        )
        ...
        self.physical_robot_sim_state["ee_pos"] = (
            self.physical_robot_sim_state["ee_pos"] + robot_action[:3]
        )
    ...
    def _get_subgoal_and_gripper(self, obs):
        ...
        if self.task_phase == "approach":
            ...
            if self.initial_block_pos is not None:
                subgoal = self.initial_block_pos + np.array(
                    [0, 0, 0.20]
                )  # Lift 20cm up
            else:
            ...
            if (
                self.initial_block_pos is not None
                and ee_pos[2] > self.initial_block_pos[2] + 0.15
            ):  # 15cm above INITIAL position
                self.task_phase = "transport"
                print(
                    f"[t={self.timestep}] *** ENTERING TRANSPORT PHASE - MPC WILL OPTIMIZE TO MOVE BLOCK TO ZONE B ***"
                )
            ...
            horizontal_block_to_zone = np.linalg.norm(
                red_block_pos[:2] - zone_b_pos[:2]
            )
            if (
                horizontal_block_to_zone < 0.10
            ):  # Within 10cm - relaxed threshold to exit before gradients vanish
    ...
    def get_robot_action(self, obs):
        ...
        if use_mpc:
            ...
            action = self._mpc_to_subgoal(
                obs, subgoal, gripper_state, self.robot_planner
            )
        else:
        ...
        if self.timestep % 20 == 0:
            ...
            feature_str = ", ".join(
                [f"{name}={val:.3f}" for name, val in zip(feature_names, features)]
            )

            if self.task_phase in ["grasp", "release"]:
                wait_time = self.timestep - (
                    self.grasp_start_time
                    if self.task_phase == "grasp"
                    else self.release_start_time
                )
                print(
                    f"[t={self.timestep}] Phase: {self.task_phase} (waiting... {wait_time}/80), "
                ...
                if self.initial_block_pos is not None:
                    ...
                    print(
                        f"[t={self.timestep}] Phase: {self.task_phase}, "
    ...
    def get_expert_action(self, obs):
        ...
        if use_mpc:
            ...
            action = self._mpc_to_subgoal(
                obs, subgoal, gripper_state, self.expert_planner
            )
    ...
    def get_action(self, obs):
        ...
        if self.is_intervention():
            if not self.recording:
                ...
                print(
                    f"[Timestep {self.timestep}] Started intervention: '{self.utterance}'"
                )
            ...
            self.robot_trajectory.append(
                {"obs": copy.deepcopy(robot_obs), "control": robot_action.copy()}
            )

            # Record human trajectory (actual state + expert action)
            self.human_trajectory.append(
                {"obs": copy.deepcopy(obs), "control": expert_action.copy()}
            )
            ...
            control_tf = tf.constant(
                robot_action[:3], dtype=tf.float32
            )  # Only position control

            new_ee_pos, new_ee_vel = arm_dynamics_step(
                ee_pos_tf, ee_vel_tf, control_tf, self.dt
            )
            ...
            if self.task_phase in ["lift", "transport"]:
                ...
                grasp_offset = self.robot_sim_state["red_block_pos"] - (
                    robot_obs["ee_pos"]
                )
                self.robot_sim_state["red_block_pos"] = (
                    self.robot_sim_state["ee_pos"] + grasp_offset
                )
            ...
            if self.recording:
                print(
                    f"[Timestep {self.timestep}] Ended intervention, updating weights..."
                )
                print(
                    f"  Robot trajectory (counterfactual): {len(self.robot_trajectory)} states"
                )
                print(
                    f"  Expert trajectory (actual): {len(self.human_trajectory)} states"
                )
    ...
    def get_feature_descriptions(self):
        ...
        """
        return feature_utils.get_feature_descriptions(
            include_red_dist=False, include_zones=True
        )
