...
def car_dynamics_step(x, y, v, angle, acc, ang_vel, dt, friction):
    total_acc = acc - friction * v**2 * tf.math.sign(v)
...
def batched_next_car_state(
    ...
    if len(state.shape) != 2 or state.shape[1] != 4:
        raise ValueError("Input state has incorrect length {}".format(control.shape))
    if len(control.shape) != 2 or control.shape[1] != 2:
        raise ValueError("Input control has incorrect shape".format())

    x, y, v, angle = state[:, 0], state[:, 1], state[:, 2], state[:, 3]
    acc, ang_vel = control[:, 0], control[:, 1]

    new_x, new_y, new_v, new_angle = car_dynamics_step(
        x, y, v, angle, acc, ang_vel, dt, friction
    )
...
def next_car_state(
    ...
    if len(state) != 4:
        raise ValueError("Input state has incorrect length {}".format(len(state)))
    if len(control) != 2:
        raise ValueError("Input control has incorrect length {}".format(len(control)))
...
@tf.function
def next_car_trajectory(
    state: tf.Tensor,
    controls: Union[tf.Tensor, list[tf.Tensor]],
    dt: float,
    dynamics_fn,
) -> tf.Tensor:
    """
    Computes a trajectory of car states by applying the given controls one after the other.

    Args:
        state: A tf.Tensor of shape (4,), of the form
                    [x, y, v, angle],
                where:
                    - x: the x coordinate
                    - y: the y coordinate
                    - v: the instantaneous velocity (in units/second)
                    - angle: the angle of the car in radians (0 is due west)
        controls: A tf.Tensor of shape (num_steps, 2), where each row is of the form
                    [acc, angle_vel],
                where:
                    - acc: the instantaneous change in velocity
                    - angle_vel: the angular velocity of the car
        dt: the unit of time to step forward for.
        friction: the per timestep decrease in velocity due to friction (in 1/units)

    Returns:
        trajectory: a tf.Tensor of shape (num_steps, 4), where each row is of the form
                    [x, y, v, angle],
                representing the state after each tick of the simulation. This array will
                not include the initial state.
    """

    trajectory = []
    for i in range(len(controls)):
        state = dynamics_fn(state, controls[i], dt)
            ...
            numpy_or_tf_state: array-like object (e.g. a tf.Tensor or np.array)
                    ...
                    This should be a rank 1 tensor of the form [x, y, v, angle]:
                        ...
                        - angle: the angle of the car in radians
                                (where 0 is due west)
        """
        self.state = tf.Variable(numpy_or_tf_state, name="state", dtype=tf.float32)
...
class CarControl(object):
    def __init__(self, numpy_or_tf_controls):
        ...
        """
        self.control = tf.Variable(
            numpy_or_tf_controls, name="control", dtype=tf.float32
        )
    ...
    def _check_control_is_valid(self):
        if self.control.shape[0] != 2:
            raise ValueError(
                "Input control has incorrect shape {}".format(self.control.shape)
            )
