...
class CVPlanner(CarPlanner):
    ...
    def __init__(
        ...
        self.control_low = -1.5
        self.control_high = 1.5
    ...
    def influence_prediction(self):
        return np.zeros(
            [self.NC * self.horizon, self.NC * self.horizon], dtype=np.float32
        )
    ...
    def compute_grads(self, init_state):
        ...
        with tf.GradientTape() as tape:
            ...
            for t in range(self.horizon):
                ...
                world_state = batched_next_car_state(
                    world_state, controls, self.world.dt
                )

                rrs.append(
                    self.world.cars[self.ri].reward_fn(
                        world_state,
                    )
                )
        ...
        gradient = tape.gradient(rr, self.robot_control)
    ...
    def optimize_plan(
        self,
        init_state: Union[None, tf.Tensor, tf.Variable, np.array] = None,
        ...
        for i in range(self.n_iter):
            ...
            # Project back into the control limits
