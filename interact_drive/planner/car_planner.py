...
class CarPlanner:
    ...
    def __init__(self, world: CarWorld, car: Car) -> None:
        self.world = world
        self.car = car
    ...
    def setup_planner(
        self, h_index, horizon: int, n_iter: int, plan_for_human: bool
    ) -> None:
        """
    ...
    def compute(self, process_name, init_state=None, void_graph_setup=False):
        ...
        while self.next_process_index < process_index:
            self.planning_processes[self.next_process_index](
                self.init_state, void_graph_setup
            )
    ...
    def report_run(self, func_key):
        ...
        if self.world.verbose:
            print(
                f"Car {self.car.index + 1}/{len(self.world.cars)} {self.name} {func_key} Graph Set Up"
            )
