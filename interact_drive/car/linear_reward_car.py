...
class LinearRewardCar(Car):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        super().__init__(
            env, init_state, color=color, opacity=opacity, friction=friction, **kwargs
        )
        self.weights_tf = tf.Variable(weights, dtype=tf.float32)
    ...
    def weights(self, weights):
        self.weights_tf.assign(weights)

    @tf.function
    def reward_fn(self, state, weights=None):
