...
class Car:
    ...
    def __init__(
        self,
        env: "CarWorld",
        init_state: Union[np.ndarray, tf.Tensor, list[float]],
        color: str,
        opacity: float = 1.0,
        friction: float = 0.2,
        index: int = 0,
        ...
        init_state_tf = tf.constant(init_state, dtype=tf.float32)
        assert isinstance(init_state_tf, tf.Tensor)
        assert init_state_tf.shape == (4,)
        self.init_state = init_state_tf
        self.state = init_state_tf
    ...
    def set_next_control(
        self, control: Union[None, np.ndarray, tf.Tensor, Iterable] = None
    ):
        """
